#!/usr/bin/env bash
###############################################################################
# Planet&Travel – EMBED GMB MAP IN FOOTER
# Adds a Google Maps iframe with your business address and a "Review us" link
###############################################################################
set -euo pipefail
cd "$(dirname "$0")" || exit 1
...
# ── 1. Update the Footer with GMB embed ───────────────────────────────
...
npm run build
git add -A
git commit -m "📍 Embed GMB map in footer" || echo "Nothing to commit"
git push origin main

echo ""
echo "✅ Footer updated with:"
echo "   - Google Maps iframe showing your exact location"
echo "   - 'Review us on Google' link (replace YOUR_PLACE_ID with your real Place ID)"
echo "   - All existing luxury footer columns preserved"