...
set -euo pipefail
cd "$(dirname "$0")" || exit 1
echo "🤍 Applying pure luxury white background..."
git pull --rebase origin main 2>/dev/null || { git stash; git pull --rebase origin main; git stash pop 2>/dev/null || true; }

# Replace the dopamine body gradient with a subtle, elegant white loop
...
body {
  ...
  color: #1a1a2e;
  /* Pure luxury white gradient – subtle & looping */
  background: linear-gradient(135deg, #ffffff 0%, #fafafa 25%, #f5f5f5 50%, #fafafa 75%, #ffffff 100%);
  background-size: 400% 400%;
  animation: whiteLuxury 12s ease infinite;
}
@keyframes whiteLuxury {
...
.glass { background: rgba(255,255,255,0.25); backdrop-filter: blur(20px); border: 1px solid rgba(0,0,0,0.08); border-radius: 16px; }
.glass-light { background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border: 1px solid rgba(0,0,0,0.05); border-radius: 16px; }
...
.hero-fallback-bg { background: linear-gradient(135deg, #ffffff 0%, #e8e8e8 100%); background-size:400% 400%; animation: heroGradient 12s ease infinite; }
@keyframes heroGradient { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }

.header-bg { background: rgba(255,255,255,0.92); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(0,0,0,0.06); }
...
.service-card::before {
  ...
  background-size: 400% 400%; animation: borderGlow 4s linear infinite;
...
npm run build
git add -A && git commit -m "🤍 Pure luxury white background" && git push origin main
echo "✅ Done. Site now has a refined white-looping background."